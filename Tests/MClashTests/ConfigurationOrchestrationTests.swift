...
struct ConfigurationOrchestrationTests {
    ...
    @Test func startupAutomaticallyAdoptsPopulatedUnifiedConfigurationOnce() async throws {
        ...
        var initialDocument = ConfigurationDocument.mclashDefault()
        let retainedRule = RoutingRule(
            enabled: false,
            priority: 5,
            action: .direct
        )
        initialDocument.rules = [retainedRule]
        initialDocument.workspaces[0].ruleIDs = [retainedRule.id]
        let baseWorkspace = initialDocument.workspaces[0]
        let migrationWorkspace = Workspace(
            name: "Migration target",
            proxyGroupIDs: baseWorkspace.proxyGroupIDs,
            ruleIDs: [retainedRule.id],
            dnsPolicyID: baseWorkspace.dnsPolicyID,
            entranceIDs: baseWorkspace.entranceIDs
        )
        initialDocument.workspaces.append(migrationWorkspace)
        initialDocument.currentWorkspaceID = migrationWorkspace.id
        try await ConfigurationStore(layout: layout).save(initialDocument)
        ...
        #expect(model.configurationDocument.rules.count >= 9)
        #expect(model.configurationDocument.rules.contains { rule in
            rule.matchers.contains {
                if case .application("com.example.client") = $0 { return true }
                return false
            }
        })
        #expect(model.configurationDocument.rules.contains { $0.id == retainedRule.id })
        #expect(model.configurationDocument.workspaces[0].ruleIDs.contains(retainedRule.id))
    ...
    @Test func failedUnifiedStartupMigrationDoesNotMarkOrReplaceLegacyRuntime() async throws {
        ...
        let legacyRule = try CaptureRule(
            id: "legacy-startup-rule",
            priority: 10,
            action: .direct
        )
        let captureStore = try NetworkCaptureConfigurationStore(profileLayout: layout)
        let previousCapture = try await captureStore.replaceRules(
            [legacyRule],
            enabled: true,
            dnsEnabled: false
        )
        ...
        let restoredCapture = try await captureStore.load()
        #expect(restoredCapture.snapshot.rules == previousCapture.snapshot.rules)
        #expect(restoredCapture.enabled == previousCapture.enabled)
        #expect(restoredCapture.dnsEnabled == previousCapture.dnsEnabled)
        #expect(restoredCapture.snapshot.revision > previousCapture.snapshot.revision)
    }
    ...
    @Test func compilerPreservesMihomoDomainWildcardSemantics() throws {
        var document = ConfigurationDocument.mclashDefault()
        let group = try #require(document.proxyGroups.first)
        let rule = RoutingRule(
            priority: 10,
            matchers: [.domainWildcard("*.service.example.com")],
            action: .proxyGroup(group.id)
        )
        document.rules = [rule]
        ...
        #expect(yaml.contains("DOMAIN-WILDCARD,*.service.example.com,MClash Select"))
        #expect(!yaml.contains("DOMAIN-KEYWORD,.service.example.com"))
    }

    @Test func compilerDoesNotEmitJSONSlashEscapesIntoMihomoYAML() throws {
        var document = ConfigurationDocument.mclashDefault()
        let group = try #require(document.proxyGroups.first)
        let rule = RoutingRule(
            priority: 10,
            matchers: [.ipCIDR("192.0.2.0/24"), .transport("tcp")],
            action: .proxyGroup(group.id)
        )
        document.rules = [rule]
        document.workspaces[0].ruleIDs = [rule.id]
        ...
        #expect(yaml.contains("IP-CIDR,192.0.2.0/24"))
