...
struct AutomationCommandGatewayTests {
    ...
    func runtimeDiagnosticsAreRedactedAndReadOnly() async throws {
        ...
        #expect(result["geoDatabases"]?.objectValue?["ip"]?.objectValue?["status"]?.stringValue != nil)
        #expect(result["geoDatabases"]?.objectValue?["site"]?.objectValue?["status"]?.stringValue != nil)
