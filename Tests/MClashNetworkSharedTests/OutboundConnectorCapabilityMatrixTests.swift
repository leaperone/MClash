...
struct OutboundConnectorCapabilityMatrixTests {
    ...
    @Test("Distinguishes native VLESS TCP from WebSocket and Reality fallback")
    func classifiesVLESSTransports() throws {
        let targets = [
            try OutboundNodeTarget(protocolName: "vless", host: "tcp.example", port: 443),
            try OutboundNodeTarget(protocolName: "vless", host: "ws.example", port: 443, parameters: ["network": "ws"]),
        ...
        let entries = OutboundConnectorCapabilityMatrix.entries(for: catalog)
        #expect(entries.map(\.support) == [.native, .legacyFallback, .legacyFallback])
