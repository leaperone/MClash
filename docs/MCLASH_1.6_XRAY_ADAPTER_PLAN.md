...
Unsupported nodes stay visible with a reason and cannot silently fall back to a different backend. The renderer covers VLESS, VMess, Trojan, Shadowsocks, HTTP, HTTPS, SOCKS5, and Hysteria2. Each protocol needs both schema validation and a traffic probe before interoperability is claimed. TUIC, unknown plugins, and unsupported transport options are not silently approximated.
